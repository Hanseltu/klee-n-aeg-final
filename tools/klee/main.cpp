...
/* Jiaqi */
#include "../../lib/Core/ExecutionState.h"//should not expose this??
#include <sys/mman.h>
#include <sys/stat.h>
#include <string.h>
#include <fcntl.h>
...
/* Jiaqi */
struct kn_indica* kn_indicator;
struct HeapAlloc* nme_buf;
FILE* req_dump_fp;
unsigned long n_heap_l;
unsigned long n_heap_h;
unsigned long n_heap_mask;
int heap_idx;
...
char nme_path[] = "/home/jqhong/klee-nme/klee-nme-aeg/nme-related/";
/* /Jiaqi */
...
int main(int argc, char **argv, char **envp) {
    ...
    char tmp_path[100];
    char  shar_file_p[] = "laucher/shar_file";
    ...
    // int tmp_ret = access (tmp_path, W_OK);
    // if (tmp_ret != 0)
    // {
    //     printf ("errno: %d. \n", errno);
    // }
    ...
    int ret = fallocate(kn_shar_fd, 0x0, 0x0, 0x1000);
    if (ret == -1)
    {
    ...
    // printf ("pid: %d. \n", getpid());
    // sleep(0x100000);
    // memset (kn_shar_mem, 0x0, 0x10);
    // *((char*)kn_shar_mem) = 0x00;
    // printf ("0... \n");
    void* tmp_ptr = memset (kn_shar_mem, 0x0, 0x1000);
    printf ("1... return of memset: %p. \n", tmp_ptr);
    kn_indicator = (struct kn_indica*) kn_shar_mem;
    nme_buf = (struct HeapAlloc*)(kn_shar_mem + sizeof(struct kn_indica));
    // nme_buf = (struct HeapAlloc*) (kn_shar_mem + 0x8);
    oflow_k = (struct of_k*) nme_buf;
    oflow_n = (struct of_n*) nme_buf;
    nme_store = (uint8_t*) (kn_shar_mem + 0xff0);
    // kn_indicator->flag = 4;
    
    /* launch launcher */
    char new_buf[4];
    sprintf (new_buf, "%x", kn_shar_fd);
    pid_t fpid = fork();
    ...
    /* setup the native heap boundary */
    n_heap_mask = 0x100000000000;
    n_heap_l = 0x500000000000 + n_heap_mask;//native heap addr range is 0x600000000000-0x6fffffffffff. 
    n_heap_h = n_heap_l + 0xfffffffffff;
    // heap_idx = 0;
    /* / */
    
    char req_dump_path[100];
    char req_dump_p[] = "laucher/req_dump.txt";
    path_len = strlen(nme_path) + strlen(req_dump_p);
    if (path_len >= 100)
    {
        printf ("no sufficient space for req dump file path. path len: %d. \n", path_len);
        return -1;
    }
    strcpy(req_dump_path, nme_path);
    strcat(req_dump_path, shar_file_p);
    req_dump_fp = fopen(req_dump_path, "w+");
    if (req_dump_fp == NULL)
    {
        printf ("open req dump failed. path: %s. \n", req_dump_path);
        return -1;
    }
    printf ("req_dump file ptr: %p. \n", req_dump_fp);
    ...
    //     // if (i > 10000000000)
    //     //     break;
    //     // // printf ("klee read flag: %d. \n", kn_indicator->flag);
    // } while (kn_indicator->flag != 0);
    // printf ("klee waits onsite initialization. kn_indicator->flag, addr: %p, value: %d. i: %d.\n", &kn_indicator->flag, kn_indicator->flag, i);
    // kn_indicator->flag = 1;
    // close(kn_shar_fd);
    // fclose(req_dump_fp);
    // return 0;
    // kn_indicator->flag = 1;
    // printf ("onsite initialization done, klee starts to execute. \n");
    /* / */
    /* /Jiaqi */
